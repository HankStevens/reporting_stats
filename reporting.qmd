...
## Convey biological meaning {#sec-convey}
...
### Should you show us your data?
The subfigures A, D, G are all based on the same seven x,y pairs, but we add progressively more information from A to D to G. The same is true for the center column using 1000 x,y pairs with normally distributed *y* (B, E, H), and for the right column using 1000 x,y pairs but with lognormally distributed *y* (C, F, G).
...
### Dynamite plots are plots that can blow up in your face. 

See, for instance,

[<u>https://simplystatistics.org/posts/2019-02-21-dynamite-plots-must-die/</u>](https://simplystatistics.org/posts/2019-02-21-dynamite-plots-must-die/){target="_blank"}


## Beyond p-values

*Before* we go beyond *p*-values, we should state that **a *p*-value^[as usually used in an ANOVA or t-test] is the probability that the data are consistent with a particular null hypothesis.** A *p*-value is not designed to tell us how big an effect is.^[There are entirely different approaches for describing the size of an effect.] 

Authors sometimes use the expression "marginal effect" to refer to an effect with a *p*-value slightly above your favorite criterion, such as $p<0.05$. This confounds the expected size of the effect, with the confidence interval of the effect, and the *p*-value from a test. A better use of the term "marginal effect" might be, 

- a very small effect of dubious ecological importance,
- the change in a response due to a one unit change in a predictor (e.g., the slope of a regression line). 
- an effect which *marginalizes* (i.e., averages over) other effects.

**Given the limitation of *p*-values, we are better prepared to go beyond them.**

 In 2019, The American Statistical Association devoted an issue of their journal, *The American Statistician*, on [<u>“Moving to a World> Beyond ‘p \<> 0.05’”</u>](https://doi.org/10.1080/00031305.2019.1583913){target="_blank"}. They state,
...
We strongly encourage you to use the link above to access the opening editorial for that issue of the journal. All that their advice entails, and what practical steps we take if we no longer can say “statistically significant”, is waaaay beyond the scope of this short document. 

Nonetheless, one step we can all take now is to ***convey biological meaning*** when we report our results. See @sec-convey.
...
Ecology and Evolution*, 15, 266–281.](https://doi-org.proxy.lib.miamioh.edu/10.1111/2041-210X.14270){target="_blank"}
...
1. How you estimated the parameters of the model (i.e., ran the model)
   a. State the method (MCMC, quadratic approximation, etc.)
...
1. How you checked assumptions of the *model* (e.g., error distribution and homogeneity of variances, etc.)
   ...
   a. Scaled residual plots and tests of assumptions e.g., homogeneity of variances using scaled residuals. For help with this in R, consider these packages:
      a. Lüdecke et al., 2021. performance: An R Package for Assessment,
  Comparison and Testing of Statistical Models. Journal of Open Source
  Software, 6(60), 3139. https://doi.org/10.21105/joss.03139.
      a. Hartig, F., 2024. DHARMa: Residual Diagnostics for Hierarchical (Multi-Level/Mixed) Regression Models. doi:10.32614.
...
# Citing R and added packages
 ...
 If you use R, use the correct citation and include it in your *references cited* section.
 ...
 *If you generate statistical results using a package other than the included **stats** package, you should cite it.* To cite an R package other than the base package or stats package, type citation(package=“package_name”), e.g.,
 ...
 Include all relevant citations with your other references.
...
# In fine
The main point of statistical modeling is to help us understand the world around us, and sometimes other worlds too. When you are finished with your modeling, and write about what your modeling told you, remember to 

1. tell your reader what you did,
1. convince your reader that what you did was appropriate and reasonable,
1. convey what you learned in terms of the real world, and 
