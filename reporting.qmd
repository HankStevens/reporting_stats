...
Put into either the methods methods text or table, supplement, or a mix of these the following:
...
1. How you estimated the parameters of the model (i.e., ran the model)
   ...
   a. Cite the core R or Python version (citation() # for R Core Team)
   a. Cite any packages that were central to the analysis (but not the data wrangling or graphics), e.g., citation(package=“rethinking”) # for rethinking e.g.
   a. The number of chains, iterations per chain, and the amount of warm-up.
1. How you checked or diagnosed the estimation procedure (e.g., convergence)
   a. For parameters: R-hat, effective sample size (bulk and tail ESS)
   a. Overall for chains, tracerank plots or graphical test of uniformity (Sailynoja et al. 2022).
1. How you checked assumptions of the model (e.g., homogeneity of variances, etc.)
